...
import { Lock, Play, Trophy, Terminal, Cpu, Code2, Sparkles, Activity, Loader2, CheckCircle2, ArrowLeft } from "lucide-react";
...
const EventRow = ({ day, date, status }: EventRowProps) => {
  ...
  return (
    <div className={`group relative flex flex-col md:flex-row items-center justify-between p-6 rounded-xl border transition-all duration-500 ${
      isActive 
        ...
        : "border-white/5 bg-white/[0.02] opacity-50 filter grayscale"
    }`}>
      ...
      <div className="flex items-center gap-4">
        ...
        {isLocked ? (
          ...
          <Link 
            href={`/arena/day/${status === "active" || status === "past" ? "day_" + day : "#"}`}
...
export default function ArenaPage() {
  ...
  const STAGES_ORDER = ["playground", "day_1", "day_2", "day_3", "day_4", "day_5", "day_6", "day_7"];
  ...
  const isPlaygroundLocked = activeStage !== "playground" && activeStage !== "loading";
  ...
  return (
    <div className="min-h-screen bg-[#111111] text-white selection:bg-[#FFC72C] selection:text-black">
      <div className="max-w-7xl mx-auto px-6 py-12">
        ...
        {/* Playground Section */}
        <section className={`mb-20 transition-all duration-700 ${isPlaygroundLocked ? "opacity-40 grayscale" : ""}`}>
          <div className="flex flex-col md:flex-row md:items-center gap-4 mb-8">
            <div className="flex items-center gap-4 flex-1">
              <Cpu className={`${isPlaygroundLocked ? "text-white/20" : "text-[#FFC72C]"} w-6 h-6`} />
              <h2 className={`text-xl font-bold uppercase tracking-widest ${isPlaygroundLocked ? "text-white/40" : ""}`}>
                The Playground
              </h2>
              ...
              <div className="flex items-center gap-2 bg-white/5 p-1 rounded-lg border border-white/10">
                <button
                  onClick={() => !isPlaygroundLocked && handleLevelChange("beginner")}
                  disabled={isPlaygroundLocked}
                  className={`px-3 py-1 rounded text-[10px] font-bold uppercase transition-all ${
                    level === "beginner" ? "bg-[#FFC72C] text-black" : "text-white/40 hover:text-white"
                  } ${isPlaygroundLocked ? "cursor-not-allowed" : ""}`}
                >
                  Beginner
                </button>
                <button
                  onClick={() => !isPlaygroundLocked && handleLevelChange("experienced")}
                  disabled={isPlaygroundLocked}
                  className={`px-3 py-1 rounded text-[10px] font-bold uppercase transition-all ${
              ...
              <span className={`text-[10px] font-mono border px-2 py-1 rounded ${
                ...
                {isPlaygroundLocked ? "PAST STAGE" : "UNLOCKED"}
              </span>
            </div>
          ...
          {activeStage === "playground" ? (
            ...
            <div className="p-8 rounded-xl border border-white/5 bg-white/[0.02] text-center">
              <p className="text-white/20 font-mono uppercase tracking-widest text-sm italic">
                The playground is now closed. The Main Event is in progress.
              </p>
            </div>
          )}
        </section>
        ...
        <section>
          <div className="flex items-center gap-4 mb-8">
            ...
            <span className={`text-[10px] font-mono border px-2 py-1 rounded ${
              ...
              {activeStage === "playground" ? "RESTRICTED" : "ACTIVE"}
            </span>
          </div>
